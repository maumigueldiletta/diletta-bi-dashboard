...
export function KpiCard({
  label, value, delta, deltaLabel, unit, accent = "default", highlight = false,
}: {
  label: string;
  value: string;
  delta?: number;
    ...
    <div className={cn(
      ...
      <div className="mt-2 flex items-baseline gap-2">
        <div className={cn(
          ...
          accent === "red" ? "text-diletta-red" : accent === "green" ? "text-diletta-green" : "text-diletta-text1"
        )}>
          {value}
        </div>
        {unit && <div className="text-sm text-diletta-text3">{unit}</div>}
      </div>
      {delta !== undefined && (
        <div className={cn(
